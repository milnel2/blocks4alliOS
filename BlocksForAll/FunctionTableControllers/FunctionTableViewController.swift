...
class FunctionTableViewController: UITableViewController {
    ...
    var functionNames: [String] = []  // All the names of the functions a user creates placed in an array instead of dictionary so has a set order
    ...
    override func viewDidLoad() {
        ...
        functionNames = Array(currentFunctionDict.keys)
    ...
    @IBAction func insertFunction(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "Done".localized, style: .default, handler: {action in
            ...
            if self.validateFunctionName(name: textField.text!, currentAlert: alert) {
                ...
                self.functionNames.append(textField.text!)
                self.currentFunctionDict.updateValue([], forKey: self.functionNames[self.functionNames.count - 1])
                let insertionIndexPath = NSIndexPath(row: self.functionNames.count-1, section: 0)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.nameButton.setTitle(functionNames[indexPath.row], for: .normal)
        ...
        cell.nameButton.accessibilityLabel = String.localizedStringWithFormat(formattedString, functionNames[indexPath.row])
    ...
    func removeMainWorkspace(){
        functionNames = functionNames.filter {$0 != "Main Workspace"}
    }
    
    /// Deletes a row from functions menu and gets rid of this function's values
    func deleteCell(cell: UITableViewCell) {
        if let deletionIndexPath = tableView.indexPath(for: cell) {
            oldKey.append(functionNames[deletionIndexPath.row])
            ...
            let yes = UIAlertAction(title: NSLocalizedString("Yes", comment: ""), style: .default, handler: { (action) -> Void in
                self.currentFunctionDict.removeValue(forKey: self.functionNames[deletionIndexPath.row])
                self.functionNames.remove(at: deletionIndexPath.row)
                self.tableView.deleteRows(at: [deletionIndexPath], with: .automatic)
                
                // Remove deleted function blocks from main workspace
                for function in self.currentFunctionDict.keys{
                    for block in self.currentFunctionDict[function]!{
                        for oldFunctionName in self.oldKey{
                            if block.name == oldFunctionName{
                                self.currentFunctionDict[function]!.remove(at: self.currentFunctionDict[function]!.firstIndex{$0 === block}!)
                            }
                        }
                    }
                }
    ...
    func renameCell(cell: UITableViewCell) {
        if let renameIndexPath = tableView.indexPath(for: cell) {
            oldKey.append(functionNames[renameIndexPath.row])
            let val = currentFunctionDict[functionNames[renameIndexPath.row]]
        // Show alert
        ...
        alert.addTextField { (textField) in
            ...
            if self.validateFunctionName(name: textField.text!, currentAlert: alert) {
                ...
                self.functionNames.append(textField.text!)
                self.newKey = self.functionNames[self.functionNames.count - 1]
                self.currentFunctionDict.updateValue(val!, forKey: self.functionNames[self.functionNames.count - 1])
                let insertionIndexPath = NSIndexPath(row: self.functionNames.count-1, section: 0)
                self.tableView.insertRows(at: [insertionIndexPath as IndexPath], with: .automatic)
                ...
                self.currentFunctionDict.removeValue(forKey: self.functionNames[renameIndexPath.row])
                self.functionNames.remove(at: renameIndexPath.row)
    ...
    func validateFunctionName(name: String, currentAlert: UIAlertController) -> Bool{
        ...
        if (dictionary[name] != nil) {
            ...
            return false
        } else if (self.functionNames.contains(name))  {
    ...
    @objc func blockModifier(cell: UITableViewCell, sender: UIButton!) {
        ...
        currentWorkspace = functionNames[(functionIndexPath?.row)!]
